  ...
  /* USER CODE BEGIN 2 */
	...
	if (firmware_flag_set || rcv_buff == NEW_FIRMWARE || curr_config.first_boot == FIRST_BOOT_TRUE)
	{
		...
		uint32_t new_app_addr = (curr_config.first_boot == FIRST_BOOT_TRUE) ? APP_SLOT0_ADDR :
								(curr_config.curr_app_slot == APP_SLOT0_ADDR) ? APP_SLOT1_ADDR : APP_SLOT0_ADDR;
		...
		else
		{
			...
			// printf("first boot: %x\r\n", curr_config.first_boot);
			// printf("crc slot0: %x\r\n", curr_config.slot0_crc);
			// printf("crc slot1: %x\r\n", curr_config.slot1_crc);
			// printf("appslot0: %lx\r\n", curr_config.curr_app_slot);
